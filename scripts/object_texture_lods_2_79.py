...
# Function for Blender Render
def internal_lod():
   
    char = ["BooBoo", "HaHa", "MiMi"]
    high = []
    low = []
    mats = []
    mats_low = []
    ...
    for m in mats:
        ...
        im_low = mat.texture_slots[0].texture.image.copy()
        im_low.name = mat.texture_slots[0].texture.image.name.split(".png")[0] + "_low.png"
        
        filepath_low = mat.texture_slots[0].texture.image.filepath.split("textures")[0] + "textures/low/" + mat.texture_slots[0].texture.image.filepath.split("textures")[1].split(".png")[0] + "_low" + ".png"
    ...
    for i in range(len(high)):
        bpy.data.objects[high[i]].select = True
        bpy.context.scene.objects.active = bpy.data.objects[high[i]]
            ...
            for ob in bpy.context.selected_objects:
                for mat in ob.data.materials:
                    if mat.name not in l:
                        ...
                        if mat.texture_slots[0] is not None and mat.texture_slots[0].texture.type == "IMAGE":
                            bpy.context.scene.render.engine = "CYCLES"
                            texture.image = mat.texture_slots[0].texture.image
                        bpy.context.scene.render.engine = "CYCLES"
...
class texture_lods_panel(bpy.types.Panel):
    ...
    def draw (self, context):
        ...
        layout = self.layout
        box = layout.box()
        box.operator(OBJECT_OT_texture_lods.bl_idname, text = 'Create LODs', icon = "IMAGE_COL")
        
        box = layout.box()
        row = box.row()
        row.label(text = "Create Shadeless Material", icon = "MATCAP_24")
        row = box.row()
        row.prop(context.scene, "mat_enum", expand = True)
        row = box.row()
        sh_mat = row.operator(OBJECT_OT_create_shadeless_material.bl_idname, text = 'Create', icon = "MATERIAL")
